...
def record_and_transcribe():
    ...
    with sr.Microphone() as source:
        ...
        audio = recognizer.listen(source, timeout=5, phrase_time_limit=20)  # 20 seconds recording limit
...
# Function to display feedback based on accuracy
def display_feedback(accuracy):
    if 80 <= accuracy <= 100:
        st.balloons()  # Balloons for high accuracy
    elif 60 <= accuracy < 80:
        st.snow()  # Snow for moderate accuracy
    elif 40 <= accuracy < 60:
        st.toast("Better luck next time!")  # Toast for low accuracy
    elif 1 <= accuracy < 40:
        st.toast("Can be done well!")  # Toast for very low accuracy
...
def main():
    ...
    if st.button("Record Answer"):
        ...
        if st.session_state.user_answer and "Sorry" not in st.session_state.user_answer and "Error" not in st.session_state.user_answer:
            ...
            display_feedback(st.session_state.accuracy)  # Show feedback based on accuracy
        else:
            st.session_state.show_correct_answer = False
